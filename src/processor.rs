...
fn process_initialize_stake_account(
    ...
    let stake_state = next_account_info(accounts_iter)?;
    let system_program = next_account_info(accounts_iter)?;

    let (stake_state_pda, bump_seed) = Pubkey::find_program_address(
        &[user.key.as_ref(), nft_token.key.as_ref()],
        program_id
    );
    ...
    let mut account_data = UserStakeInfo::try_from_slice(&stake_state.data.borrow()).unwrap();
